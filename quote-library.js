...
function emitChange(){window.dispatchEvent(new CustomEvent('lpp:data-changed'))}
...
function active(){return quotes.find(q=>q&&q.id===activeId)||null}
...
function setLegacy(items){
  const arr=Array.isArray(items)?clone(items):[];
  localStorage.setItem(LEGACY_KEY,JSON.stringify(arr));
  try{
    if(typeof state!=='undefined')state.quote=clone(arr);
    if(typeof renderQuote==='function')renderQuote();
  }catch(e){console.error('quote-library view:',e)}
...
function migrate(){
  ...
  if(!Array.isArray(quotes)||!quotes.length){
    let old=[];
    try{if(typeof state!=='undefined'&&Array.isArray(state.quote))old=clone(state.quote)}catch{}
    if(!old.length)old=load(LEGACY_KEY,[]);
    const q={id:uid(),name:'Wycena 1',status:'active',items:Array.isArray(old)?old:[],createdAt:now(),updatedAt:now(),completedAt:null};
    quotes=[q];activeId=q.id;save(false);
  }
  if(!quotes.some(q=>q&&q.id===activeId))activeId=quotes[0].id;
  setLegacy(active()?.items||[]);
  save(false);
}

function syncCurrent(notify=true){
  if(applying)return;
  let current=[];
  try{current=typeof state!=='undefined'&&Array.isArray(state.quote)?state.quote:load(LEGACY_KEY,[])}catch{current=load(LEGACY_KEY,[])}
  const snap=JSON.stringify(current||[]);
  if(snap===lastSnapshot)return;
  const q=active();if(!q)return;
  q.items=clone(current||[]);q.updatedAt=now();lastSnapshot=snap;
  localStorage.setItem(LEGACY_KEY,snap);
  save(notify);updateHeader();
}

function setActive(id){
  syncCurrent();
  const q=quotes.find(x=>x&&x.id===id);if(!q)return;
  activeId=id;save();setLegacy(q.items||[]);updateHeader();closeLibrary();
}
function createQuote(){
  syncCurrent();
  const n=quotes.length+1;
  const name=prompt('Nazwa nowej wyceny:',`Wycena ${n}`);if(name===null)return;
  const q={id:uid(),name:(name.trim()||`Wycena ${n}`),status:'active',items:[],createdAt:now(),updatedAt:now(),completedAt:null};
  quotes.unshift(q);activeId=q.id;save();setLegacy([]);updateHeader();
}
function renameQuote(id){
  const q=quotes.find(x=>x&&x.id===id);if(!q)return;
  const name=prompt('Nowa nazwa wyceny:',q.name||'Wycena');if(name===null||!name.trim())return;
  q.name=name.trim();q.updatedAt=now();save();renderLibrary();updateHeader();
}
function renameActive(){const q=active();if(q)renameQuote(q.id)}
function toggleDone(id){
  syncCurrent();const q=quotes.find(x=>x&&x.id===id);if(!q)return;
...
function deleteQuote(id){
  ...
  const q=quotes.find(x=>x&&x.id===id);if(!q||!confirm(`Usunąć wycenę „${q.name}”?`))return;
  quotes=quotes.filter(x=>x.id!==id);
  if(activeId===id){activeId=quotes[0].id;setLegacy(active()?.items||[])}
  save();renderLibrary();updateHeader();
}

function updateHeader(){
  const q=active();if(!q)return;
  const title=document.getElementById('activeQuoteName');const badge=document.getElementById('activeQuoteStatus');
  if(title)title.textContent=q.name||'Wycena';
  if(badge){const done=q.status==='done';badge.textContent=done?'ZREALIZOWANO':'W TRAKCIE';badge.className='quote-status '+(done?'done':'active')}
}
function renderLibrary(){
  ...
  const rows=quotes.filter(q=>(!query||String(q.name||'').toLocaleLowerCase('pl').includes(query))&&(filter==='all'||q.status===filter))
    .sort((a,b)=>a.status===b.status?new Date(b.updatedAt||0)-new Date(a.updatedAt||0):(a.status==='done'?1:-1));
  if(!rows.length){list.innerHTML='<div class="ql-empty">Brak wycen pasujących do filtrów.</div>';return}
  list.innerHTML=rows.map(q=>`<article class="ql-card ${q.status==='done'?'is-done':''} ${q.id===activeId?'is-current':''}" data-id="${esc(q.id)}"><div class="ql-main"><div class="ql-title-row"><strong>${esc(q.name||'Wycena')}</strong><span class="ql-badge ${q.status==='done'?'done':'active'}">${q.status==='done'?'ZREALIZOWANO':'W TRAKCIE'}</span>${q.id===activeId?'<span class="ql-current">AKTUALNA</span>':''}</div><div class="ql-meta">${(q.items||[]).length} pozycji • ${qty(q).toLocaleString('pl-PL')} szt. • ${fmtMoney(total(q))}</div><div class="ql-date">Ostatnia zmiana: ${fmtDate(q.updatedAt)}${q.completedAt?` • Zrealizowano: ${fmtDate(q.completedAt)}`:''}</div></div><div class="ql-actions"><button class="secondary ql-open" data-id="${esc(q.id)}">${q.id===activeId?'Otwarta':'Otwórz'}</button><button class="secondary ql-done" data-id="${esc(q.id)}">${q.status==='done'?'Przywróć':'Zrealizowano'}</button><button class="ghost ql-rename" data-id="${esc(q.id)}">Zmień nazwę</button><button class="delete-btn danger ql-delete" data-id="${esc(q.id)}" title="Usuń">×</button></div></article>`).join('');
